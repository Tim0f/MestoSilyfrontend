...
import ButtonSvg from '../assets/svg/button.svg?react'; // раскомментировано
    ...
    <div className="w-full min-h-screen bg-customblack text-customwhite p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {products.map((item) => (
          <div
            key={item.id}
            className="relative w-full max-w-[442px] mx-auto max-h-[500px] textured-border overflow-hidden p-[10px] flex flex-col"
          >
            {/* КАРТИНКА + ЦЕНА */}
            ...
            <div className="flex flex-col flex-1 p-[25px]">
              ...
              <div
                onClick={() => buy(item.id)}
                className="relative w-full cursor-pointer mt-auto flex justify-center"
              >
                <ButtonSvg
                  width={233}
                  height={81}
                  className="fill-customyellow"
                />
                <span className="absolute inset-0 flex items-center justify-center text-customblack font-p">
                  купить
                </span>
              </div>
            </div>
